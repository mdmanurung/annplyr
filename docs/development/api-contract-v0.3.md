...
| `accessor.tally` | selected metadata axis | optional metadata weight | pandas DataFrame | no persistent grouping | input order or descending count when sort | result table independent | metadata only; backed safe | no matrix reads | selection, column, axis |
...
| `accessor.pull` | source-dependent | obs, var, X/layer, raw, obsm, varm, obsp, varp, uns | pandas Series or DataFrame | ungrouped | requested source and selector order | independent pandas result | dense, sparse, DataFrame, and backed projections | projected budget | selection, source, column, budget |
...
| `accessor.nest_by` | selected metadata axis | obs/var metadata selectors | pandas DataFrame containing AnnData subsets | ephemeral keys | first-seen key order | nested subsets are non-mutating views | sparse/backed behavior follows AnnData slicing | no matrix evaluation reads | selection, column, axis, name repair |
...
| `export.FeaturePresence` | n/a | found and missing collections | immutable report object | n/a | input order | independent | n/a | n/a | constructor validation |
